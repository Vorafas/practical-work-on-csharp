...
namespace menu
{
    class Program
    {
        ...
        static string GetStirng(string txt) // Обработка строки и привидение ее к массиву
        {
            string text;
            do
            {
        ...
        static bool GetCheckString(string text) {
            bool flag = false;
        ...
        static string[] GetChangedArr(string text) {
            text = text.Trim();
        ...
        static string[] GetChangedArray(string[] array) { // Смена предложений местами
            string[] newArr = new string [array.Length - 1];
            if (newArr.Length - 1 <= 0)
            {
                newArr[0] = "Слишком короткое предложение";
            }
            else
            {
        ...
        static void GetStringArr(string[] array) // Получение строки из массива
        {
            string text = "";
            ...
            do {
                ...
                switch (number) {
                    ...
                    case 2:
                        ...
                        if (GetCheckString(str))
                        {
                            string[] arr = GetChangedArr(str);
                            string[] newArr = GetChangedArray(arr);
                            GetStringArr(newArr);
                        }
                        else {
                            Console.WriteLine("Не является предложением");
                        }                        
                        break;
                    case 3:
                        GetText("1. Удалить элемент из массива. \n2. Поменять местами элементы массива.\n3. Вывести меню. \n4. Выход");
